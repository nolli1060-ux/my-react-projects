            ...
            {items.map((dish) => (
              <li key={dish.id}>
                <div className="cart-item-info">
                  ...
                  <div className="quantity-controls" aria-label={`Quantity for ${dish.name}`}>
                    <button type="button" onClick={() => decrease(dish.id)} aria-label={`Decrease ${dish.name} quantity`}>
                      -
                    </button>
                    <span>{dish.quantity}</span>
                    <button type="button" onClick={() => increase(dish.id)} aria-label={`Increase ${dish.name} quantity`}>
                      +
                    </button>
                  </div>
                </div>
                <strong>Subtotal: {dish.price * dish.quantity} ETB</strong>
